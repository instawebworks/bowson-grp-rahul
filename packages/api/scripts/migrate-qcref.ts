/**
 * Migration: qcRef field on tickets (QC reference captured at the
 * QC Check → Packing gate), then reload PostgREST.
 * Run: pnpm --filter @bowson/api tsx scripts/migrate-qcref.ts
 */
...
const SQL = `
alter table "tickets" add column if not exists "qcRef" text;
...
try {
  ...
  console.log('✅ tickets qcRef column added + PostgREST reloaded (direct pg)');
} catch (err) {
  console.warn(`direct pg connection failed (${(err as Error).message}) — trying Kong /pg/query…`);
  await runViaMeta();
  console.log('✅ tickets qcRef column added + PostgREST reloaded (via /pg/query)');
}
process.exit(0);
