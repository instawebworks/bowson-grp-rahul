/**
 * Migration: dayHrs on operatives (per-week day-hour overrides for the
 * Production Planner), then reload PostgREST.
 * Run: pnpm --filter @bowson/api tsx scripts/migrate-dayhrs.ts
 */
...
const SQL = `
alter table "operatives" add column if not exists "dayHrs" jsonb not null default '{}'::jsonb;
...
try {
  ...
  console.log('✅ operatives dayHrs column added + PostgREST reloaded (direct pg)');
} catch (err) {
  console.warn(`direct pg connection failed (${(err as Error).message}) — trying Kong /pg/query…`);
  await runViaMeta();
  console.log('✅ operatives dayHrs column added + PostgREST reloaded (via /pg/query)');
}
process.exit(0);
