/**
 * Migration: packing-checklist fields on orders (hardware checklist saved at
 * the Packing gate + free-form packing notes), then reload PostgREST.
 * Run: pnpm --filter @bowson/api tsx scripts/migrate-packing.ts
 */
...
const SQL = `
alter table "orders" add column if not exists "packingChecklist" jsonb;
alter table "orders" add column if not exists "packingNotes"     text;
...
try {
  ...
  console.log('✅ orders packing columns added + PostgREST reloaded (direct pg)');
} catch (err) {
  console.warn(`direct pg connection failed (${(err as Error).message}) — trying Kong /pg/query…`);
  await runViaMeta();
  console.log('✅ orders packing columns added + PostgREST reloaded (via /pg/query)');
}
process.exit(0);
