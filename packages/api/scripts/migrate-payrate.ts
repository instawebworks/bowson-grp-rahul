/**
 * Migration: payRate on operatives (hourly rate shown on the
 * Operatives cards), then reload PostgREST.
 * Run: pnpm --filter @bowson/api tsx scripts/migrate-payrate.ts
 */
...
const SQL = `
alter table "operatives" add column if not exists "payRate" double precision;
...
try {
  ...
  console.log('✅ operatives payRate column added + PostgREST reloaded (direct pg)');
} catch (err) {
  console.warn(`direct pg connection failed (${(err as Error).message}) — trying Kong /pg/query…`);
  await runViaMeta();
  console.log('✅ operatives payRate column added + PostgREST reloaded (via /pg/query)');
}
process.exit(0);
